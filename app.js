...
function renderCharts(c){
 ...
 const kabLabels=kabData.map(x=>x[0]);
 const kabValues=kabData.map(x=>x[1]);
 if(kabChart)kabChart.destroy();
 kabChart=new Chart($("kabChart"),{type:"bar",data:{labels:kabLabels,datasets:[{label:"Jumlah DESTANA",data:kabValues}]},options:{indexAxis:"y",responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>fmt(ctx.raw)+" DESTANA"}}},scales:{x:{beginAtZero:true,ticks:{precision:0}},y:{ticks:{font:{size:9}}}}}});
 ...
 const fundLabelsPlugin={
   id:"fundTotals",
   afterDatasetsDraw(chart){
     const meta=chart.getDatasetMeta(0), values=chart.data.datasets[0].data;
     ...
     meta.data.forEach((arc,i)=>{
       ...
       const pct=fundTotal?Math.round(v/fundTotal*100):0;
       if(arc.outerRadius-arc.innerRadius<22)return;
       ctx.font="700 11px Arial"; ctx.textAlign="center"; ctx.textBaseline="middle";
       ctx.fillStyle="#fff"; ctx.strokeStyle="rgba(0,0,0,.18)"; ctx.lineWidth=3;
       ctx.strokeText(String(v),x,y); ctx.fillText(String(v),x,y);
       if(pct>=8){
         ctx.font="600 9px Arial"; ctx.strokeStyle="rgba(0,0,0,.16)"; ctx.lineWidth=2;
         ctx.strokeText(pct+"%",x,y+13); ctx.fillText(pct+"%",x,y+13);
       }
     }); ctx.restore();
   }
 };
 fundChart=new Chart($("fundChart"),{type:"doughnut",data:{labels:fund.map(x=>x[0]),datasets:[{data:fund.map(x=>x[1])}]},plugins:[fundLabelsPlugin],options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom",labels:{font:{size:9},boxWidth:12},generateLabels(chart){const ds=chart.data.datasets[0],labels=chart.data.labels||[];return labels.map((label,i)=>({text:label+" ("+fmt(ds.data[i])+")",fillStyle:Chart.defaults.backgroundColor,strokeStyle:"#fff",lineWidth:1,hidden:false,index:i}))}}}}});

 const yearly={}; const fundNames=[];
 R.forEach(r=>{
   if(c.status<0||c.tahun<0||!statusIs(r[c.status],"PEMBENTUKAN"))return;
   const y=String(r[c.tahun]??"").trim(); if(!/^\d{4}$/.test(y))return;
   const f=String(r[c.sumber]??"").trim(); if(!validValue(f))return;
   if(!yearly[y])yearly[y]={}; yearly[y][f]=(yearly[y][f]||0)+1;
 ...
 const totalLabelsPlugin={id:"yearTotals",afterDatasetsDraw(chart){const{ctx,scales}=chart;ctx.save();ctx.font="700 12px Arial";ctx.textAlign="center";ctx.textBaseline="bottom";years.forEach((year,idx)=>{const total=yearly[year]?Object.values(yearly[year]).reduce((a,b)=>a+b,0):0;if(!total)return;const x=scales.x.getPixelForValue(idx);const y=scales.y.getPixelForValue(total)-6;ctx.fillStyle="#0f2742";ctx.fillText(total.toLocaleString("id-ID"),x,y)});ctx.restore()}};
...
function showAllKabupaten(){
  const c=cols();

  // Hanya Kabupaten/Kota yang memiliki nama valid.
  const data=Object.entries(countBy(c.kab))
    .filter(([name])=>validValue(name))
    .sort((a,b)=>b[1]-a[1]);

  const title=$("allKabTitle");
  const canvas=$("allKabChart");
  if(!canvas)return;

  title.textContent="Sebaran DESTANA Seluruh Kabupaten/Kota Jawa Tengah";

  const old=Chart.getChart(canvas);
  if(old)old.destroy();

  // Landscape: seluruh Kabupaten/Kota berada di sumbu horizontal.
  ...
  new Chart(canvas,{
    ...
    options:{
      ...
      layout:{padding:{top:24,left:12,right:12,bottom:10}},
      plugins:{
        legend:{display:false},
        tooltip:{callbacks:{label:ctx=>fmt(ctx.raw)+" DESTANA"}}
      },
      scales:{
        x:{
          ticks:{
  ...
  $("allKabModal").classList.add("show");
}
...
$("closeModal").onclick=()=>$("detailModal").classList.remove("show");$("showAllKab").onclick=showAllKabupaten;$("closeAllKab").onclick=()=>$("allKabModal").classList.remove("show");$("allKabModal").onclick=e=>{if(e.target.id==="allKabModal")$("allKabModal").classList.remove("show")};$("detailModal").onclick=e=>{if(e.target.id==="detailModal")$("detailModal").classList.remove("show")};document.addEventListener("keydown",e=>{if(e.key==="Escape"){$("detailModal").classList.remove("show");$("allKabModal").classList.remove("show")}});
