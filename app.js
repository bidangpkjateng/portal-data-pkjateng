...
function load(){
  ...
  const url=`https://docs.google.com/spreadsheets/d/${CFG.id}/gviz/tq?tqx=responseHandler:${callback}&sheet=${encodeURIComponent(CFG.sheet)}`;
  window[callback]=function(data){
    try{
      if(!data || data.status!=="ok") throw new Error("Google Sheets status bukan OK");
      H=data.table.cols.map(c=>c.label||c.id||"");
      R=data.table.rows.map(row=>H.map((_,i)=>{const cell=row.c&&row.c[i];return cell?(cell.f??cell.v??""):"";}));
