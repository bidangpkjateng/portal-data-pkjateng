const CFG = {
  csvUrl: "https://docs.google.com/spreadsheets/d/e/2PACX-1vTSVMYiWmJ4Kx9BeHpmVWDP-ac0htrRgvHR2zhFo2IZY6SMbicfJs5JsEyro6klFk2pzdoRVdQ3Fup_/pub?gid=2090181256&single=true&output=csv"
};
...
async function load() {
  ...
  try {
    ...
    if (!r.ok) throw Error("Gagal mengunduh CSV");
    const t = await r.text();
    
    const data = parseCSV(t);
    if (data.length < 1) throw Error("Data kosong");

    H = data[0].map(h => h.trim());
    R = data.slice(1).filter(row => row.some(cell => cell.trim() !== ""));
    ...
    toast("Gagal membaca data dari Google Sheets.");
  }
}
